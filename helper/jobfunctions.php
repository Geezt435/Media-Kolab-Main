...
function getCategoryJobNameFromId($idcategory)
{
    try {
        $conn = getConnection();
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sql = "SELECT category_name FROM tb_category_job_vacancy WHERE category_id = :categoryId";
        $request = $conn->prepare($sql);
...
function uploadImageJob($logoName, $imageFile)
{
...
function deleteImageJob($logoName)
{
    $api = new UploadApi();
    if (!is_null($logoName)) {
        try {
            $api->destroy($logoName);
            $api->destroy($logoName);
        } catch (PDOException $errorMessage) {
            $error = $errorMessage->getMessage();
            echo $error;
